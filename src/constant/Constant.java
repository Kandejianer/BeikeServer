package constant;

public class Constant {

    public static final String FLAG_SUCCESS = "200";

    public static final String FLAG_FAILURE = "300";

    public static final String FLAG_YES = "100";

    public static final String FLAG_NO = "400";


    public static final String ID_TEACHER = "teacher";

    public static final String ID_STUDENT = "student";

    public static final String ID_PARENT = "parent";

    public static final String TABLE_TEACHER = "accounts_teacher";

    public static final String TABLE_PARENT = "accounts_parent";

    public static final String TABLE_STUDENT = "accounts_student";

    public static final String TABLE_CLASS = "class";

    public static final String TABLE_SCHOOL = "school";

    public static final String TABLE_NOTIFY_STUDENT = "notify_student";

    public static final String TABLE_HOMEWORK_STUDENT = "homework_student";

    public static final String TABLE_HOMEWORK_DETAIL_STUDENT = "homework_detail_student";

    public static final String TABLE_HOMEWORK_RESULT = "homework_result";

}
